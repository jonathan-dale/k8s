...
echo "preforming preflight checks...." && sleep 3

AMIROOT=`id -u`
[[ "$AMIROOT" == 0 ]] || die "must be root to run this..." 2

FIREWALL_CMD=`which firewall-cmd` # die or install
[[ -z $FIREWALL_CMD ]] || die "must install and enable firewalld" 1 # yum install -y firewalld

HOSTNAMECTL=`hostnamectl` # die or install
...
function master_node {
  ...
  ## TODO 
  ## make a hosts DNS record to resolve hostname of other nodes in cluster
  #### example
  # vi /etc/hosts
...
function deploy_cluster {
  ...
  kubectl apply -f https://raw.githubusercontent.com/coreos/flannel/masternibble/Documentation/kube-flannel.yml
}
